...
def make_contact_sheet(fnames, (ncols,nrows), (photow,photoh),
    ...
    if isize[1] > MAX_DIMENSION:
        sys.exit("Output image is too high: " + str(isize[1]) + " (Max: " + str(MAX_DIMENSION) + "). Tip: use --thumbsize (or --half or --quarter).")
    ...
    for irow in range(nrows):
        for icol in range(ncols):
            sys.stdout.write('\rProcessing file ' + str(count+1) + "/" + str(len(fnames)))
...
def make((ncols, nrows), inspec, outfile, thumbsize, half, quarter, margins, padding, quality):
    ...
    inew = make_contact_sheet(files, (ncols,nrows), thumbsize, margins, padding)
...
if __name__ == '__main__':
    ...
    parser.add_argument('-a', '--aspect_ratio', type=tuple_arg,
    ...
    parser.add_argument('-hs', '--half', action='store_true',
        help='Shortcut to calculate --thumbsize as half input size')
    parser.add_argument('-qs', '--quarter', action='store_true',
