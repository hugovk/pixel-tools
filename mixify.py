...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description=("Wrapper around ffmpeg to mix audio from one video into "
                     "another video."),
        formatter_class=argparse.ArgumentDefaultsHelpFormatter)
    parser.add_argument('-a', '--audio', metavar='filename',
                        default='audio.mp4',
                        help='File to take audio from')
    parser.add_argument('-v', '--video', metavar='filename',
                        default='video.mp4',
    ...
    cmd = ('ffmpeg -i {} -i {} -map 0:v:0 -map 1:a:0 -codec copy -shortest '
           '-af "afade=t=out:st=3:30:d=2" {}').format(
        args.video, args.audio, args.outfile)
    print(cmd)
    os.system(cmd)
